...
exports.getModules = async (req, res) => {
    try {
        ...
        const [lessons] = await db.query('SELECT id, module_id, title, video_hls_path, support_material_path, display_order FROM lessons ORDER BY display_order ASC');
...
exports.createLesson = async (req, res) => {
    try {
        const { module_id, title, video_hls_path, display_order } = req.body;
        const support_material_path = req.file ? `/uploads/materials/${req.file.filename}` : null;

        const [result] = await db.query(
            'INSERT INTO lessons (module_id, title, video_hls_path, support_material_path, display_order) VALUES (?, ?, ?, ?, ?)',
            [module_id, title, video_hls_path, support_material_path, display_order || 0]
        );
